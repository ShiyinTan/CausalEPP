...
#define item_num 26823   //64443  pre: 26813  cur: 26823
#define user_num 48820   //75258  pre: 48799  cur: 48820


double** item_popularity = (double **)malloc(item_num * sizeof(double*));
double** grad = (double **)malloc(item_num * sizeof(double*));
int** item_interaction = (int **)malloc(item_num * sizeof(int*));
int** user_interaction = (int **)malloc(user_num * sizeof(int*));
...
void load_popularity(py::array_t<double>& tau_p)
{
	...
	if ((fp = fopen("./data/Douban-movie/item_interactions.csv", "r")) != NULL)                                  
	{ 
		for (i = 0; i < item_num; i++)
		{
			...
			//printf("itr_num_item = %d\n", itr_num_item);
			if ((item_interaction[i] != NULL) && (i != 0))
			{
				//free(item_interaction[i]);
				item_interaction[i] = NULL;
			}
			if ((item_popularity[i] != NULL) && (i != 0))
			{
...
void load_user_interation_val()
{
	...
	if ((fp = fopen("./data/Douban-movie/train_list.txt", "r")) != NULL)                                //*
	{
		while (!feof(fp))
		{
			line = fgets(buffer, sizeof(buffer), fp);
...
void load_user_interation_test()
{
	...
	int itr_num_user, item_id;
	// printf("loading user interaction\n");
	if ((fp = fopen("./data/Douban-movie/train_list_t.txt", "r")) != NULL)                         //*
	{
		while (!feof(fp))
		{
			line = fgets(buffer, sizeof(buffer), fp);
...
PYBIND11_MODULE(pybind_douban, m) {
