...
def quality_loss(f_i, f_j, q_i, q_j):
    ...
    quality_loss = -torch.log(torch.sigmoid(sgn*(q_i-q_j))+1e-6)
    return quality_loss
...
def main():
    ...
    model = model(user_num, item_num, opt.emb_dim, min_time, max_time, device)
    model.to(device)
    optimizer = get_initializer(model)
    evaluator = Evaluator(model, user_num, item_num, max_time, device)
    ...
    if opt.test_only:
        print(f"further testing: topk {opt.topk}")
        print(f"model path: {opt.model_path_to_load}")
    ...
    for epoch in range(opt.epochs):
        ...
        for user, item_i, item_j, timestamp, split_idx in train_loader:
            ...
            if opt.method == 'IPS':
                ...
                if opt.method in ['CausalEPP']:
                    frequency_i = torch.from_numpy(model.item_frequency_overall[item_i.cpu().numpy()]).to(model.device)
                    frequency_j = torch.from_numpy(model.item_frequency_overall[item_i.cpu().numpy()]).to(model.device)
                    q_loss = quality_loss(frequency_i, frequency_j, model.q[item_i], model.q[item_j])
        ...
        if epoch - evaluator.best_perf['best_epoch'] > 50 and epoch > 100:
            with open(opt.log_path, 'a+') as f_log:
    ...
    with open(opt.log_path, 'a+') as f_log:
        ...
        f_log.write(
            "mrr = {:.5f}".format(evaluator.best_perf['mrr'][0]))
    print(
        "End. Best epoch {:03d}: recall = {:.5f}, precision = {:.5f}, NDCG = {:.5f}, "
    ...
    print(
        " recall@3 = {:.5f}, precision@3 = {:.5f},".format(
            evaluator.best_perf['recall@3'][0],
            evaluator.best_perf['precision@3'][0]))
    print("mrr = {:.5f}".format(evaluator.best_perf['mrr'][0]))
    return evaluator.best_perf
...
def test_all_log_file_through_k():
    ...
    opt.test_only = True
    metric = 'recall'
    dict_keys = ['recall', 'precision', 'ndcg', 'averageRating', 'recall@3', 'precision@3', 'mrr']
    datasets = ['Ciao']  # 'Amazon-CDs_and_Vinyl', 'Douban-movie', 'Amazon-Music',
    ...
    # with open(f'./perf/{opt.dataset}_{opt.method}_perf-through-k.txt', 'w') as f:
    #     f.write(f"\n")
    ...
    # opt.get_file_path()
    Evaluator.pbd_import(Evaluator)
    # path = f"./log/{dataset}"  # 文件夹目录
    path = f"./model/{opt.dataset}"  # 文件夹目录
    files = os.listdir(path)  # 得到文件夹下的所有文件名
    perf_pd = pd.DataFrame(columns=metrics)
    if opt.dataset == "Ciao":
        model_load_file_name = "xx.pth"
    elif opt.dataset == "Amazon-Music":
        model_load_file_name = "xx.pth"
    file_name = model_load_file_name
    # for file_name in files:
    ...
    if file_name[-4:] == '.pth':
        ...
        for test_method in methods:
            with open(f'./perf/{opt.dataset}_{test_method}_perf-through-k.txt', 'a+') as f:
                f.write(f"\n")
            ...
            print("----"*10)
            print(f"file_name: {file_name}, test_method: {test_method}")
            print(opt.dataset, opt.backbone, opt.method)
            ...
            for topk in topk_list:
                ...
                with open(f'./perf/{opt.dataset}_{test_method}_perf-through-k.txt', 'a+') as f:
                    ...
                    for k in perf.keys():
                        if k in dict_keys:
                            write_results[k] = f"{perf[k][0]:.5f}"
                    f.write(f"topk: {opt.topk}, perf: {write_results} \n")
    ...
    # perf_pd = perf_pd.loc[sort_index]
    perf_pd = perf_pd.round(5)
    perf_pd.to_csv('./perf/' + metric + '_' + opt.dataset + '_perf-through-k.csv', sep='\t')
    print(opt.dataset)
    print(perf_pd)
...
if opt.test_only:
    ...
    if opt.method == 'IPS':
        ...
        main_PDA()
    elif opt.method == 'TIDE' or opt.method in ['CausalEPP']:
        # main_TIDE()
        main_TIDE()
    else:
        main_base()
