...
class Data(Dataset):
    ...
    def ng_sample(self):  # 用c语言模块做负采样
        ...
        user_positive = user_positive.reshape((user_positive.shape[0], 1)).astype(np.float64)
        item_positive = np.array(
            self.features['item']).repeat(
            self.num_ng)  # 正样本item做同样的操作
        item_positive = item_positive.reshape((item_positive.shape[0], 1)).astype(np.float64)
        ...
        split_idx = np.array(
            ...
            self.num_ng)  # timestamp做同样的操作
        split_idx = split_idx.reshape((split_idx.shape[0], 1)).astype(np.float64)
        # print(user_positive.shape, item_positive.shape, time_positive.shape, 
        #       item_negative.shape, split_idx.shape)
        # print(user_positive.dtype, item_positive.dtype, time_positive.dtype, 
        #       item_negative.dtype, split_idx.dtype)
        ...
        #     axis=1)  # 将正样本和负样本合并
        ## 尝试逐步合并
        temp1 = np.concatenate((user_positive, item_positive), axis=1)
        # print("concatenate success")
        temp2 = np.concatenate((temp1, item_negative), axis=1)
        # print("concatenate success")
        temp3 = np.concatenate((temp2, time_positive), axis=1)
        # print("concatenate success")
        features_np = np.concatenate((temp3, split_idx), axis=1)
        # print("concatenate success")
        # print(features_np.dtype)
        # self.features_fill = features_np.tolist()
        self.features_fill = features_np
