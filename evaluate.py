...
class Evaluator(object):
    def __init__(self, model, user_num, item_num, max_time, device='cpu'):
        self.model = model
    ...
    def evaluate(
        ...
        result = np.zeros((len(rate_batch), 7)) # recall、precision、NDCG、average rating、recall@3、precision@3、mrr
    ...
    def test(self, users_to_test, popularity_item_np, item_quality=None, item_conformity=None):
        ...
        for u_batch_id in range(n_user_batchs):
            ...
            user_batch = torch.tensor(user_batch).to(self.device)
            item_batch = torch.tensor(item_batch).to(self.device)
            ...
            if opt.method == 'PD':
                ...
                processed_batch = rate_batch.copy()
                processed_batch[rate_batch < 20] = np.log(1+np.exp(rate_batch[rate_batch < 20]))
                # rate_batch = np.log(1 + np.exp(rate_batch))  # 激活函数
                rate_batch = processed_batch * np.tanh(popularity_item_np)
                # rate_batch = F.softplus(rate_batch) * torch.tanh(popularity_item_np)
                ...
                sensitivity_u = (self.model.user_sensitivity_all_times.iloc[user_batch.cpu().numpy(), 100].values).reshape(-1,1) # [#u, 1]
                short_term_popularity_norm_i = (self.model.item_frequency_all_times_norm.iloc[item_batch.cpu().numpy(), 100].values).reshape(1,-1) # [1, #i]

                ui_align_score = np.exp(-opt.smooth_parm_alpha * np.abs(sensitivity_u - short_term_popularity_norm_i))
                
                popularity_item_np = np.array(item_quality + ui_align_score * item_conformity.reshape(1,-1))
                processed_batch = rate_batch.copy()
                processed_batch[rate_batch < 20] = np.log(1+np.exp(rate_batch[rate_batch < 20])) # softplus
                rate_batch = processed_batch * np.tanh(popularity_item_np)
    ...
    def run_test(self, epoch):
        ...
        item_quality = None
        item_conformity = None
        popularity_item_np = None
        if opt.method == 'PDA':
            ...
            frequency = self.model.item_frequency_overall
            popularity_item_np = self.pbd.popularity(
                np.arange(
                    self.item_num), np.ones(
                    self.item_num,) * self.max_time)
            q = F.softplus(self.model.q).cpu().detach().numpy()
            b = F.softplus(self.model.b).cpu().detach().numpy()
            item_quality = q * frequency
            item_conformity = b * popularity_item_np
        else:
            popularity_item_np = 0

        ret = self.test(users_to_test, popularity_item_np=popularity_item_np, 
                        item_quality=item_quality, item_conformity=item_conformity)
        ...
        if opt.show_performance:
            util.print_str(opt.log_path, perf_str1 + perf_str2 + perf_mrr)
